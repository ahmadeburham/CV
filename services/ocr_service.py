...
def _preprocess_for_ocr(image) -> List[Tuple[str, object]]:
    ...
    # Upscale for small ID images to improve text recognition.
    h, w = gray.shape[:2]
    ...
    # Multiple OCR-friendly variants.
    ...
    # Slight sharpening for faint printed text.
...
def _clean_ocr_output(tokens: List[str]) -> List[str]:
    ...
    # Keep ordering, remove exact duplicates.
    seen = set()
    unique = []
    for item in cleaned:
        if item in seen:
...
def _run_easyocr_variants(image_variants: List[Tuple[str, object]]) -> Tuple[List[str], Optional[str], Optional[str]]:
    ...
    try:
        # Keep CPU as default for reliability in notebook/Kaggle.
    ...
    for variant_name, img in image_variants:
        try:
            ...
            tokens = [entry[1] for entry in results if len(entry) >= 2]
            ...
            # Early stop if result is strong enough.
            if score >= 14 and len(cleaned) >= 4:
                break
        except Exception:
            continue
...
def _extract_with_huggingface(image_path: str) -> Tuple[List[str], Optional[str]]:
    """Optional fallback OCR path using Hugging Face TrOCR."""
    try:
...
def extract_text(image_path: str) -> Dict:
    ...
    # If EasyOCR is too weak, optionally try HF fallback and keep the stronger result.
    hf_error = None
