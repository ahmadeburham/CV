...
def _run_easyocr_variants(
    image_variants: List[Tuple[str, object]],
) -> Tuple[List[str], Optional[str], Optional[str], List[Dict]]:
    try:
        import easyocr
    except Exception as exc:
        ...
        for paragraph_mode in (False, True):
            try:
                ...
                for entry in results:
                    if len(entry) < 3:
                        continue
                    bbox, text, confidence = entry[0], entry[1], float(entry[2])
    ...
    return best_tokens, best_variant, None, best_boxes
...
def extract_text(image_path: str) -> Dict:
    ...
    raw_text, variant_used, easyocr_error, text_boxes = _run_easyocr_variants(variants)
    engine_used = "easyocr"
