...
def _extract_raw_card_crop(image: np.ndarray, quad: np.ndarray) -> np.ndarray:
...
def process_id_image(image_path: str, template_path: str, output_dir: str) -> Dict:
    ...
    raw_card_crop = _extract_raw_card_crop(image, quad)
    rectified_card = crop_and_rectify_card(image, quad, TEMPLATE_SIZE)
